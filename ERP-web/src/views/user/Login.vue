      ...
      loginSuccess (res) {
        ...
        if(res.data && res.data.user) {
          if(res.data.user.loginName === 'admin'){
            let desc = 'admin只是平台运维用户，真正的管理员是商户(测试账号为jsh），admin不能编辑任何业务数据，只能配置平台菜单和创建商户'
      ...
      departConfirm(res, loginName){
        if(res.code==200){
          ...
          if(res.data.msgTip == 'user can login'){
            ...
            if(loginName === 'jsh') {
              err.message = 'jsh用户已停用，请注册商户进行体验！';
            } else {
              err.message = '用户所属的商户被禁用';
            }
            this.requestFailed(err)
            this.Logout();
          } else if(res.data.msgTip == 'tenant is expire'){
            err.message = '用户所属的商户已过期';
            this.requestFailed(err)
            this.Logout();
