...
import com.ywy.erp.service.materialCategory.MaterialCategoryService;
...
import com.ywy.erp.utils.BaseResponseInfo;
import com.ywy.erp.utils.ExcelUtils;
...
public class MaterialService {
        ...
        try{
            ...
            if (null != list) {
                for (MaterialVo4Unit m : list) {
                    ...
                    m.setStock(receiptsItemService.getStockByParam(null,m.getId(),null,null));
                    resList.add(m);
                }
            }
        }catch(Exception e){
            ...
            if(obj.get("stock")!=null) {
                ...
                for (int i = 0; i < stockArr.size(); i++) {
                    ...
                    if (jsonObj.get("id") != null && jsonObj.get("initStock") != null) {
                        ...
                        //更新当前库存
                        receiptsItemService.updateCurrentStockFun(material.getId(), depotId);
                    }
                }
            }
            logService.insertLog("商品",
    ...
    public int batchDeleteMaterialByIds(String ids) throws Exception{
        ...
        //校验单据子表	receipts_item
        List<ReceiptsItem> receiptsItemList =null;
        try{
            receiptsItemList=  receiptsItemMapperEx.getReceiptsItemListListByMaterialIds(idArray);
        }catch(Exception e){
            JshException.readFail(logger, e);
        }
            ...
            for(MaterialWithInitStock m: mList) {
                ...
                for(Depot depot: depotList){
                    ...
                    if(stock!=null && stock.compareTo(BigDecimal.ZERO)!=0) {
                        ...
                        //更新当前库存
                        receiptsItemService.updateCurrentStockFun(mId, depotId);
                    }
                }
            }
            info.code = 200;
    ...
    public int batchSetMaterialCurrentStock(String ids) throws Exception {
        ...
        for(Long mId: idList) {
            for(Depot depot: depotList) {
                receiptsItemService.updateCurrentStockFun(mId, depot.getId());
                res = 1;
            }
        }
        return res;
