...
import com.ywy.erp.entities.MaterialVo4Unit;
import com.ywy.erp.entities.Unit;
...
public class MaterialController {
            ...
            if (null != dataList) {
                for (MaterialVo4Unit material : dataList) {
                    ...
                    if(StringUtil.isNotEmpty(material.getSku())){
                        stock = receiptsItemService.getSkuStockByParam(depotId,material.getMeId(),null,null);
                    } else {
                        stock = receiptsItemService.getStockByParam(depotId,material.getId(),null,null);
                        if (material.getUnitId()!=null){
                            String commodityUnit = material.getCommodityUnit();
    ...
    private void getStockByMaterialInfo(MaterialVo4Unit mvo) throws Exception {
        ...
        if (StringUtil.isNotEmpty(mvo.getSku())) {
            stock = receiptsItemService.getSkuStockByParam(mvo.getDepotId(), mvo.getMeId(), null, null);
        } else {
            stock = receiptsItemService.getStockByParam(mvo.getDepotId(), mvo.getId(), null, null);
