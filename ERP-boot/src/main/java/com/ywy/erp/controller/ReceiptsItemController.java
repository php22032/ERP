...
import com.ywy.erp.service.unit.UnitService;
...
/**
 * @author ji-sheng-hua 百货商场管理系统
 */
@RestController
@RequestMapping(value = "/receiptsItem")
@Api(tags = {"单据明细"})
public class ReceiptsItemController {
    private Logger logger = LoggerFactory.getLogger(ReceiptsItemController.class);
    ...
    public String findDetailByTypeAndMaterialId(
        ...
        List<ReceiptsItemVo4DetailByTypeAndMId> list = receiptsItemService.findDetailByTypeAndMaterialIdList(parameterMap);
        JSONArray dataArray = new JSONArray();
        if (list != null) {
            for (ReceiptsItemVo4DetailByTypeAndMId d: list) {
        ...
        objectMap.put("total", receiptsItemService.findDetailByTypeAndMaterialIdCounts(parameterMap));
        ...
        try {
            ...
            if(list!=null && list.size()>0) {
                ...
                if(StringUtil.isNotEmpty(materialVo4Unit.getSku())){
                    stock = receiptsItemService.getSkuStockByParam(depotId,materialVo4Unit.getMeId(),null,null);
                } else {
                    stock = receiptsItemService.getStockByParam(depotId,materialVo4Unit.getId(),null,null);
    ...
    public BaseResponseInfo getDetailList(@RequestParam("headerId") Long headerId,
        ...
        BaseResponseInfo res = new BaseResponseInfo();
        try {
            List<ReceiptsItemVo4WithInfoEx> dataList = new ArrayList<ReceiptsItemVo4WithInfoEx>();
            if(headerId != 0) {
                dataList = receiptsItemService.getDetailList(headerId);
            }
            ...
            JSONArray dataArray = new JSONArray();
            if (null != dataList) {
                for (ReceiptsItemVo4WithInfoEx diEx : dataList) {
                    JSONObject item = new JSONObject();
                    ...
                    if(StringUtil.isNotEmpty(diEx.getSku())){
                        stock = receiptsItemService.getSkuStockByParam(diEx.getDepotId(),diEx.getMaterialExtendId(),null,null);
                    } else {
                        stock = receiptsItemService.getStockByParam(diEx.getDepotId(),diEx.getMaterialId(),null,null);
                    ...
                    item.put("finishNumber", receiptsItemService.getFinishNumber(diEx.getMaterialId(), diEx.getHeaderId())); //已入库|已出库
     ...
     * @return
     */
    public String getOtherInfo(String[] mpArr, ReceiptsItemVo4WithInfoEx diEx)throws Exception {
    ...
    public BaseResponseInfo findByAll(@RequestParam("currentPage") Integer currentPage,
        ...
        try {
            ...
            List<ReceiptsItemVo4WithInfoEx> dataList = receiptsItemService.findByAll(StringUtil.toNull(materialParam),
                    timeB,(currentPage-1)*pageSize, pageSize);
            String[] mpArr = mpList.split(",");
            int total = receiptsItemService.findByAllCount(StringUtil.toNull(materialParam), timeB);
            map.put("total", total);
            //存放数据json数组
            JSONArray dataArray = new JSONArray();
            if (null != dataList) {
                List<Long> idList = new ArrayList<>();
                for (ReceiptsItemVo4WithInfoEx m : dataList) {
                    idList.add(m.getMId());
                }
                ...
                for (ReceiptsItemVo4WithInfoEx diEx : dataList) {
                    ...
                    BigDecimal prevSum = receiptsItemService.getStockByParamWithDepotList(depotList,mId,null,timeA);
                    Map<String,BigDecimal> intervalMap = receiptsItemService.getIntervalMapByParamWithDepotList(depotList,mId,timeA,timeB);
    ...
    public BaseResponseInfo totalCountMoney(@RequestParam("depotIds") String depotIds,
        ...
        try {
            ...
            if (null != dataList) {
                for (ReceiptsItemVo4WithInfoEx diEx : dataList) {
                    Long mId = diEx.getMId();
                    BigDecimal thisSum = receiptsItemService.getStockByParamWithDepotList(depotList,mId,null,endTime);
    ...
    public BaseResponseInfo buyIn(@RequestParam("currentPage") Integer currentPage,
        ...
        try {
            ...
            String[] mpArr = mpList.split(",");
            int total = receiptsItemService.findByAllCount(StringUtil.toNull(materialParam), endTime);
            map.put("total", total);
            //存放数据json数组
            JSONArray dataArray = new JSONArray();
            if (null != dataList) {
                for (ReceiptsItemVo4WithInfoEx diEx : dataList) {
                    JSONObject item = new JSONObject();
                    BigDecimal InSum = receiptsItemService.buyOrSale("入库", "采购", diEx.getMId(), monthTime, "number");
                    BigDecimal OutSum = receiptsItemService.buyOrSale("出库", "采购退货", diEx.getMId(), monthTime, "number");
                    BigDecimal InSumPrice = receiptsItemService.buyOrSale("入库", "采购", diEx.getMId(), monthTime, "price");
                    BigDecimal OutSumPrice = receiptsItemService.buyOrSale("出库", "采购退货", diEx.getMId(), monthTime, "price");
    ...
    public BaseResponseInfo saleOut(@RequestParam("currentPage") Integer currentPage,
        ...
        try {
            ...
            if (null != dataList) {
                for (ReceiptsItemVo4WithInfoEx diEx : dataList) {
                    JSONObject item = new JSONObject();
                    BigDecimal OutSumRetail = receiptsItemService.buyOrSale("出库", "零售", diEx.getMId(), monthTime,"number");
                    BigDecimal OutSum = receiptsItemService.buyOrSale("出库", "销售", diEx.getMId(), monthTime,"number");
                    BigDecimal InSumRetail = receiptsItemService.buyOrSale("入库", "零售退货", diEx.getMId(), monthTime,"number");
                    BigDecimal InSum = receiptsItemService.buyOrSale("入库", "销售退货", diEx.getMId(), monthTime,"number");
                    BigDecimal OutSumRetailPrice = receiptsItemService.buyOrSale("出库", "零售", diEx.getMId(), monthTime,"price");
                    BigDecimal OutSumPrice = receiptsItemService.buyOrSale("出库", "销售", diEx.getMId(), monthTime,"price");
                    BigDecimal InSumRetailPrice = receiptsItemService.buyOrSale("入库", "零售退货", diEx.getMId(), monthTime,"price");
                    BigDecimal InSumPrice = receiptsItemService.buyOrSale("入库", "销售退货", diEx.getMId(), monthTime,"price");
    ...
    public BaseResponseInfo findStockWarningCount(@RequestParam("currentPage") Integer currentPage,
        ...
        try {
            ...
            String[] mpArr = mpList.split(",");
            List<ReceiptsItemStockWarningCount> list = receiptsItemService.findStockWarningCount((currentPage-1)*pageSize, pageSize, materialParam, depotList);
            //存放数据json数组
            if (null != list) {
                for (ReceiptsItemStockWarningCount disw : list) {
                    ReceiptsItemVo4WithInfoEx diEx = new ReceiptsItemVo4WithInfoEx();
            ...
            int total = receiptsItemService.findStockWarningCountTotal(materialParam, depotList);
            map.put("total", total);
            map.put("rows", list);
            res.code = 200;
            res.data = map;
    ...
    public BaseResponseInfo buyOrSalePrice(HttpServletRequest request, HttpServletResponse response)throws Exception {
        ...
        try {
            ...
            for(String month: list) {
                ...
                BigDecimal outPrice = receiptsItemService.inOrOutPrice("入库", "采购", month);
                BigDecimal inPrice = receiptsItemService.inOrOutPrice("出库", "采购退货", month);
                obj.put("x", month);
            ...
            for(String month: list) {
                JSONObject obj = new JSONObject();
                BigDecimal outPrice = receiptsItemService.inOrOutPrice("出库", "销售", month);
                BigDecimal inPrice = receiptsItemService.inOrOutPrice("入库", "销售退货", month);
                obj.put("x", month);
                obj.put("y", outPrice.subtract(inPrice));
            ...
            for(String month: list) {
                JSONObject obj = new JSONObject();
                BigDecimal outPrice = receiptsItemService.inOrOutRetailPrice("出库", "零售", month);
                BigDecimal inPrice = receiptsItemService.inOrOutRetailPrice("入库", "零售退货", month);
                obj.put("x", month);
                obj.put("y", outPrice.subtract(inPrice));
    ...
    public BaseResponseInfo getBatchNumberList(@RequestParam("name") String name,
        ...
        Map<String, Object> map = new HashMap<>();
        try {
            List<ReceiptsItemVoBatchNumberList> reslist = new ArrayList<>();
            List<ReceiptsItemVoBatchNumberList> list = receiptsItemService.getBatchNumberList(name, depotId, barCode, batchNumber);
            for(ReceiptsItemVoBatchNumberList bn: list) {
